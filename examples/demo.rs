...
use lineread::{Interface, Prompter, ReadResult};
use lineread::chars::escape_sequence;
use lineread::command::COMMANDS;
use lineread::complete::{Completer, Completion};
use lineread::inputrc::parse_text;
use lineread::terminal::Terminal;

const HISTORY_FILE: &str = "lineread.hst";

fn main() -> io::Result<()> {
    let interface = Arc::new(Interface::new("demo")?);
    let mut thread_id = 0;

    println!("This is the lineread demo program.");
    ...
    while let ReadResult::Input(line) = interface.read_line()? {
        ...
        match cmd {
            "help" => {
                println!("lineread demo commands:");
                println!();
